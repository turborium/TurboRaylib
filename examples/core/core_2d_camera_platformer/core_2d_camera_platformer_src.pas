...
begin
  ...
  Player.Position := TVector2.Create(400, 280);
  Player.Speed := 0;
  Player.CanJump := False;

  EnvItems := [
    TEnvItem.Create(TRectangle.Create(  0,   0, 1000, 400), 0, LIGHTGRAY),
    TEnvItem.Create(TRectangle.Create(  0, 400, 1000, 200), 1, GRAY),
    TEnvItem.Create(TRectangle.Create(300, 200,  400,  10), 1, GRAY),
    TEnvItem.Create(TRectangle.Create(250, 300,  100,  10), 1, GRAY),
    TEnvItem.Create(TRectangle.Create(650, 300,  100,  10), 1, GRAY),
    TEnvItem.Create(TRectangle.Create(450, 50,   100,  10), 1, BEIGE)
  ];
  ...
  begin
    ...
    BeginDrawing();
      ...
      BeginMode2D(Camera);
        ...
        PlayerRect := TRectangle.Create(Player.Position.X - 20, Player.Position.Y - 40, 40, 40);
...
begin
  Camera.Offset := TVector2.Create(Width / 2.0, Height / 2.0);
  Camera.Target := Player.Position;
end;
...
begin
  ...
  MinX := 1000; MinY := 1000; MaxX := -1000; MaxY := -1000;
    ...
    MaxV := GetWorldToScreen2D(TVector2.Create(MaxX, MaxY), Camera);
    MinV := GetWorldToScreen2D(TVector2.Create(MinX, MinY), Camera);
...
begin
  BBoxWorldMin := GetScreenToWorld2D(TVector2.Create((1 - BBox.X) * 0.5 * Width, (1 - BBox.Y) * 0.5 * Height), Camera);
  BBoxWorldMax := GetScreenToWorld2D(TVector2.Create((1 - BBox.X) * 0.5 * Width, (1 - BBox.Y) * 0.5 * Height), Camera);
  Camera.Offset := TVector2.Create((1 - BBox.X) * 0.5 * Width, (1 - BBox.Y) * 0.5 * Height);
