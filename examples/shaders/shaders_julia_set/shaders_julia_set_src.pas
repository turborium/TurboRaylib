...
begin
  ...
  InitWindow(ScreenWidth, ScreenHeight, UTF8String('raylib [shaders] example - julia sets'));

  // Load julia set shader
  // NOTE: Defining 0 (NULL) for vertex shader forces usage of internal default vertex shader
  Shader := LoadShader(nil, TextFormat(UTF8String('resources/shaders/shaders/glsl%i/julia_set.fs'), Integer(GLSL_VERSION)));
  ...
  OffsetSpeed := TVector2.Create(0.0, 0.0);

  // Get variable (uniform) locations on the shader to connect with the program
  // NOTE: If uniform variable could not be found in the shader, function returns -1
  CLoc := GetShaderLocation(Shader, UTF8String('c'));
  ZoomLoc := GetShaderLocation(Shader, UTF8String('zoom'));
  OffsetLoc := GetShaderLocation(Shader, UTF8String('offset'));

  // Tell the shader what the screen dimensions, zoom, offset and c are
  ScreenDims[0] := GetScreenWidth(); ScreenDims[1] := GetScreenHeight();
  SetShaderValue(Shader, GetShaderLocation(Shader, UTF8String('screenDims')), @ScreenDims, SHADER_UNIFORM_VEC2);
  ...
  begin
    ...
    BeginDrawing();
      ...
      BeginShaderMode(Shader);
        ...
        DrawTextureEx(Target.Texture, TVector2.Create(0.0, 0.0), 0.0, 1.0, WHITE);
      EndShaderMode();

      if ShowControls then
      begin
        DrawText(UTF8String('Press Mouse buttons right/left to zoom in/out and move'), 10, 15, 10, RAYWHITE);
        DrawText(UTF8String('Press KEY_F1 to toggle these controls'), 10, 30, 10, RAYWHITE);
        DrawText(UTF8String('Press KEYS [1 - 6] to change point of interest'), 10, 45, 10, RAYWHITE);
        DrawText(UTF8String('Press KEY_LEFT | KEY_RIGHT to change speed'), 10, 60, 10, RAYWHITE);
        DrawText(UTF8String('Press KEY_SPACE to pause movement animation'), 10, 75, 10, RAYWHITE);
      end;
    EndDrawing();
    //-------------------------------------------------------------------------------------------
  end;
