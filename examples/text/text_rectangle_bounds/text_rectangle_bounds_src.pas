...
*
********************************************************************************************)
unit text_rectangle_bounds_src;
...
procedure DrawTextBoxed(Font: TFont; Text: PAnsiChar; Rec: TRectangle; FontSize, Spacing: Single;
  WordWrap: Boolean; Tint: TColor); forward;
// Draw text using font inside rectangle limits with support for text selection
procedure DrawTextBoxedSelectable(Font: TFont; Text: PAnsiChar; Rec: TRectangle; FontSize, Spacing: Single;
  WordWrap: Boolean; Tint: TColor; SelectStart, SelectLength: Integer; SelectTint, SelectBackTint: TColor); forward;
...
begin
  ...
  InitWindow(ScreenWidth, ScreenHeight, UTF8String('raylib [text] example - draw text inside a rectangle'));
  ...
  Container := TRectangle.Create(25.0, 25.0, ScreenWidth - 50.0, ScreenHeight - 250.0);
  Resizer := TRectangle.Create(Container.X + Container.Width - 17, Container.Y + Container.Height - 17, 14, 14);
  ...
  LastMouse := TVector2.Create(0.0, 0.0); // Stores last mouse coordinates
  ...
  begin
    ...
    BeginDrawing();
      ...
      DrawTextBoxed(Font, PAnsiChar(UTF8String(Text)), TRectangle.Create(Container.X + 4, Container.Y + 4, Container.Width - 4, Container.Height - 4), 20.0, 2.0, WordWrap, GRAY);

      DrawRectangleRec(Resizer, BorderColor); // Draw the resize box

      // Draw bottom info
      DrawRectangle(0, ScreenHeight - 54, ScreenWidth, 54, GRAY);
      DrawRectangleRec(TRectangle.Create(382.0, ScreenHeight - 34.0, 12.0, 12.0), MAROON);

      DrawText(UTF8String('Word Wrap: '), 313, ScreenHeight - 115, 20, BLACK);
      if WordWrap then
        DrawText(UTF8String('ON'), 447, ScreenHeight - 115, 20, RED)
      else
        DrawText(UTF8String('OFF'), 447, ScreenHeight - 115, 20, BLACK);

      DrawText(UTF8String('Press [SPACE] to toggle word wrap'), 218, ScreenHeight - 86, 20, GRAY);

      DrawText(UTF8String('Click hold & drag the    to resize the container'), 155, ScreenHeight - 38, 20, RAYWHITE);
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          DrawRectangleRec(TRectangle.Create(Rec.X + TextOffsetX - 1, Rec.Y + TextOffsetY, GlyphWidth, Font.BaseSize * ScaleFactor), SelectBackTint);
        ...
        begin
          if IsGlyphSelected then
            DrawTextCodepoint(Font, Codepoint, TVector2.Create(Rec.X + TextOffsetX, Rec.Y + TextOffsetY), FontSize, SelectTint)
          else
            DrawTextCodepoint(Font, Codepoint, TVector2.Create(Rec.X + TextOffsetX, Rec.Y + TextOffsetY), FontSize, Tint);
        end;
      end;
